...
const Login = () => {
    ...
    //operation system
    const [OSState, OSSet] = useState("");

    let getOS = () => {
        if (navigator.appVersion.indexOf("Win") != -1) {
            return OSSet("Windows OS");
        }
        if (navigator.appVersion.indexOf("Mac") != -1)
            return OSSet("MacOS");
        if (navigator.appVersion.indexOf("X11") != -1)
            return OSSet("UNIX OS");
        if (navigator.appVersion.indexOf("Linux") != -1)
            return OSSet("Linux OS");
    }

    console.log(OSState);

    return (
        <div id="login" onLoad={getOS} >
            <div className="container">
            <ul className="nav">
            <li className="nav-item">
                <p role="button" onClick={() => history.push("/edit")}>Account</p>
            </li>
                        ...
                        <div className="col-7">
                            <div className="row">
                                <div className="col-10 mt-3">
                                    <b>Password</b>
                                    {!passEditState ? <p>Last updated 4 days ago</p> : <Password />}
                                </div>
                                <div className="col-2">
                                    <a className="nav-link" role="button" onClick={() => passChangeSet(!passEditState)}> {!passEditState ? " Update" : "Cancel"} </a>
                                </div>
                            </div>
